...
@endif
<div class="row" ng-app="barangdetail" ng-controller="returController">
	...
	<div style="background-color: #C9C9C9;" class="col-md-4">
		<div class="row">
			<div class="col-md-12">
				...
				<p>Outlet : {{ $datadetail->outlet->name }}</p>
		...
		<div class="row">
			...
			<div class="col-md-12 form-group">
				...
				<select {{$dtstatus}} ng-model="barang.product_id" class="form-control select2" name="product_id" id="productid">
					...
					<option value="{{$element->product_id}}" >{{$element->product->name}}</option>
					@endforeach
		...
		<div class="row">
			<div class="{{ $dtstatus =='disabled' ? 'hidden' : 'col-md-12'}}">
				<a href="{{ url('transaksirusak/'. $datadetail->number .'/simpan') }}" class="btn btn-default btn-block" style="margin-top: 15px; color: #636b6f; border-bottom: 5px solid #FDE088;">
					SIMPAN
				</a>
			</div>
			<div class="col-md-12">
				<a href="{{ url('barangrusak') }}" class="btn btn-default btn-block" style="margin-top: 15px; color: #636b6f; border-bottom: 5px solid #F4645F;">
					KEMBALI
				</a>
			</div>
